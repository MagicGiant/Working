...
// структура Frame
struct Frame {
    // первые четыре байта содержат имя кадра
    char frameID[4];
...
// функция отображения текущих значений 
void showFrames(FILE* f) {
    ...
    while (counter <= tagSize) {
        ...
        else if (isEqual(frame.frameID, "COMM") == true) {
            printf("%s ", frame.frameID);
            fseek(f, 4, SEEK_CUR);
            frameSize -= 4;
            while(frameSize--) {
